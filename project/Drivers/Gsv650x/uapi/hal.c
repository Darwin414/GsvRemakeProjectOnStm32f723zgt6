...
AvRet AvHalI2cReadField32(pin uint32 devAddress, pin uint32 regAddress, pin uint8 msbMask, pin uint8 lsbMask, pin uint8 lsbPos, pin uint8 endian, pin uint8 fldSpan, pout uint32 *avdata)
{
    ...
    *avdata = 0;
    ...
    if((fldSpan > 5) || (fldSpan == 0))
        return AvError;
    else
        span = fldSpan;

    ret = AvI2cRead(devAddress, regAddress, bytes, span);
    ...
    if (endian == AvBigEndian)
    {
        ...
        j = (span << 3) - lsbPos;  /* j = how many total bits */
        for (i=0; i<span; i++)
        {
            *avdata |= (((uint32)(bytes[i])) << j);
            j -= 8;
        }
        *avdata |= (uint32)((bytes[span]) >> lsbPos);
    }
    else
    {
        bytes[0] &= lsbMask;
...
AvRet AvHalI2cWriteField32 (pin uint32 devAddress, pin uint32 regAddress, pin uint8 msbMask, pin uint8 lsbMask, pin uint8 lsbPos, pin uint8 endian, pin uint8 fldSpan, pin uint32 avdata)
{
    ...
    AvI2cRead(devAddress, regAddress, bytes, span);

    if (endian == AvBigEndian)
    {
        bytes[0] = (bytes[0] & (~msbMask)) |
                       ((avdata >> (8 * (span - 1) - lsbPos)) & msbMask);
        for (i=1; i<span-1; i++)
        {
            bytes[i] = avdata >> (8 * (span - i - 1) - lsbPos);
        }
        bytes[span-1] = (bytes[span-1] & ~lsbMask) |
                        (uint8) ((avdata << lsbPos) & lsbMask);
        ret = AvI2cWrite(devAddress, regAddress, bytes, span);
    }
    else
    {
        ...
        bytes[span-1] = (bytes[span-1] & ~msbMask) |
                        ((avdata >> (8 * (span - 1) - lsbPos)) & msbMask);
        ret = AvI2cWrite(devAddress, regAddress, bytes, span);
    }
    return ret;
...
uint32 AvHalGetTimerTicks(void)
{
    uint32 ms;
    AvGetMilliSecond(&ms);
...
uint32 AvHalGetElapsedTimerTicks(uint32 start_time)
{
    uint32 ms;
    AvGetMilliSecond(&ms);
    return (uint32)(ms - start_time);
}
